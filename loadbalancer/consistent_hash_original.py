class ConsistentHashMap:
    ...
    def add_server(self, server_id, server_num=None):
        ...
        # around the ring.
        #
        # server_num is now optional: if the caller doesn't supply one
        # (or supplies None), we assign the next number from a counter
        # that only ever increments, so it's never reused after a
        # remove_server call. This keeps virtual-hash slots stable and
        # unique per server across the container's whole lifetime,
        # even if servers are removed and re-added later.
